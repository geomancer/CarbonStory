...
This fingerprint of human activity, showing a dilution of heavier carbon isotopes`^13^C and ^14^C`in the atmosphere and oceans due to burning fossil fuels, which releases carbon lacking these isotopes. It provides undeniable proof that rising`CO~2~`levels stem from fossil fuels, not natural cycles, helping attribute climate change to human actions and serving as a baseline for tracking carbon uptake by oceans and land, informing climate models and mitigation strategies. Here we see as CO2 levels rise the increasing\`^12^C from fossil fuels decreases the ^13^C/^12^C ratio:

```{r}
# ============================================================
# 10a_Keeling_Suess_Facet.R
#
# Purpose:
#   Facet the classic Keeling CO2 curve over the descending
#   13C (Keeling–Suess effect) record.
#
# Inputs:
#   data/co2_monthly_mlo_clean.rds  (cols: year, month, decimal_year, co2_ppm, trend, ndays, sdev, unc)
#   data/c13_monthly_mlo_clean.rds  (from 02k_Keeling_Suess_ETL.R)
#
# Output:
#   images/keeling_suess_facet_mlo.png
# ============================================================
...
c13_path <- "data/c13_monthly_mlo_clean.rds"

stopifnot(file.exists(co2_path))
stopifnot(file.exists(c13_path))

co2_raw <- readRDS(co2_path)
c13_raw <- readRDS(c13_path)

# ---- Build CO2 series using co2_ppm ---------------------------------------

if (!all(c("year", "month", "co2_ppm") %in% names(co2_raw))) {
  stop("co2_monthly_mlo_clean.rds must contain 'year', 'month', and 'co2_ppm' columns.")
}
...
co2 <- co2_raw %>%
  mutate(
    ...
    value = as.numeric(co2_ppm),
    series = "Atmospheric CO2 (ppm)"
  ) %>%
  filter(!is.na(date), !is.na(value)) %>%
  select(date, value, series)

# ---- Build 13C series using c13_filled ------------------------------------

if (!all(c("year", "month", "c13_filled") %in% names(c13_raw))) {
  stop("c13_monthly_mlo_clean.rds must contain 'year', 'month', and 'c13_filled' columns.")
}
...
c13 <- c13_raw %>%
  mutate(
    ...
    month = as.integer(month),
    date  = make_date(year, month, 15L),
    value = as.numeric(c13_filled),
    series = "δ13C in CO2 (‰)"
  ) %>%
  filter(!is.na(date), !is.na(value)) %>%
  select(date, value, series)

# ---- Combine + order facets -----------------------------------------------

both <- bind_rows(co2, c13) %>%
  mutate(
    series = factor(
      series,
      levels = c("Atmospheric CO2 (ppm)", "δ13C in CO2 (‰)")
    )
  ) %>%
  arrange(series, date)

# (Optional) to show only overlapping period (from 1980 on), uncomment:
# both <- both %>% filter(date >= as.Date("1980-01-01"))

# ---- Plot: Keeling–Suess facet -------------------------------------------

carbon_red <- "#C00000"  # Carbon Story themed red

p <- ggplot(both, aes(x = date, y = value)) +
  geom_line(linewidth = 0.7, color = carbon_red) +
  facet_wrap(
    ~ series,
    ncol = 1,
    scales = "free_y"
  ...
  labs(
    title    = "Keeling–Suess Effect at Mauna Loa",
    subtitle = "Rising atmospheric CO₂ and declining δ¹³C from Scripps flask records",
    x        = "Year",
    y        = NULL,
    caption  = "Source: Scripps CO₂ Program (Keeling & Walker, flask archive 21-Feb-2025)"
  ) +
...
There's a strong positive correlation: as CO~2~ levels and global temperatures rise, the frequency and cost of billion-dollar climate disasters (like hurricanes, floods, droughts) also significantly increase, demonstrating that rising greenhouse gases are directly linked to escalating economic burdens from extreme weather events, according to studies analyzing U.S. data from 1980-2021.

Looking across the full record of annual U.S. billion-dollar disaster losses and global temperature anomalies, a simple linear model shows that each additional 1°C of global warming is associated with roughly \[S\] billion dollars in extra disaster losses per year. That translates to about \[S×0.1\] billion dollars for every 0.1°C of warming, or about \[S/1.8×0.1\] billion dollars for every 0.1°F. In this framework, cooling by the same increments would be expected to reduce average losses by a similar amount. Here, we look at the accumulated cost of the recorded billion dollar events since 1980.
